...
s.linkInternalFilters = "javascript:,beachcamera.com"
...
function s_doPlugins(s) {
    ...
    /* Get external campaign param */
    if (!s.campaign)
        s.campaign = s.getQueryParam('omid');

    /* Cross Visit Participation */
    if (!s.eVar14)
        s.eVar14 = s.crossVisitParticipation(s.campaign, 's_cvp_cmp', '90', '5', '>', 'purchase');

    /* Get internal campaign param */
    if (!s.eVar5)
        s.eVar5 = s.getQueryParam('cti')

    /* Copy search term, browse category and filters to props */
    if (!s.prop8)
        s.prop8 = s.eVar7;
    ...
    /* Get cart add method for direct to cart */
    if (s.getQueryParam('cam') == 'dtc') {
        s.eVar13 = "Direct to Cart";
    } else if (!s.eVar13) {
        s.eVar13 = "Product Page";
    }
    /*
...
s.visitorNamespace = "beachcamera"
/* s.trackingServer="asset.beachcamera.com"
s.trackingServerSecure="assets.beachcamera.com"
s.vmk="49AAA833" */
s.dc = 122
