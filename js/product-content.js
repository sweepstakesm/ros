(function() {
    ...
    function _loadSyndi(siteCode, cpi) {
        ...
        window.SYNDI.push({
            "contentCallback": _loadLegacyWC
        });

        // load of the syndi library
        (function(y, n, di, go) {
                di = document.createElement(y);
                di.type = 'text/java' + y;
                di.async = true;
                di.src = n + Math.floor(Date.now() / 86400000);
                if (di.addEventListener) di.addEventListener('error', function() {
                    _loadLegacyWC(false);
                });
                if (di.attachEvent) di.attachEvent('onerror', function() {
                    _loadLegacyWC(false);
                });
                go = document.getElementsByTagName(y)[0];
    ...
    function RegisterCallbacks(callback, options) {
        if (options) {
            if (options.hasPowerPageContentCallback && typeof options.hasPowerPageContentCallback === 'function')
                _callbacks.push({
                    type: "hasPowerPageContentCallback",
                    callback: options.hasPowerPageContentCallback
                });
            if (options.noPowerPageContentCallback && typeof options.noPowerPageContentCallback === 'function')
                _callbacks.push({
                    type: "noPowerPageContentCallback",
                    callback: options.noPowerPageContentCallback
                });
        }
    ...
    window.Webcollage.loadContent = function(siteCode, cpi, options, callback) {
        _functionCalls.push({
            f: "loadContent",
            arguments: [siteCode, cpi, options, callback]
        });
    ...
    window.Webcollage.loadProductContent = function(siteCode, cpi, contentPackages) {
        _functionCalls.push({
            f: "loadProductContent",
            arguments: [siteCode, cpi, contentPackages]
        });
    ...
    window.Webcollage.loadProductContentByWcpc = function(siteCode, wcpc, moduleCode, environment, contentPackages) {
        _functionCalls.push({
            f: "loadProductContentByWcpc",
            arguments: [siteCode, wcpc, moduleCode, environment, contentPackages]
        });
        _loadSyndi(siteCode);
    }
    ...
    window.Webcollage.loadProductContentForProductListing = function(siteCode, options) {
        _functionCalls.push({
            f: "loadProductContentForProductListing",
            arguments: [siteCode, options]
        });
        _loadSyndi(siteCode);
    }
    ...
    window.Webcollage.loadProductContentForProductListingByWcpc = function(siteCode, moduleCode, environment, options) {
        _functionCalls.push({
            f: "loadProductContentForProductListingByWcpc",
            arguments: [siteCode, moduleCode, environment, options]
        });
        _loadSyndi(siteCode);
    }
    ...
    window.Webcollage.loadSectionsContentByWcpc = function(wcpc, siteCode, moduleCode, environment, type, callback) {
        _functionCalls.push({
            f: "loadSectionsContentByWcpc",
            arguments: [wcpc, siteCode, moduleCode, environment, type, callback]
        });
        RegisterCallbacks(callback);
        _loadSyndi(siteCode);
    }
